<template>
    <v-layout align-content-start mx-0 my-0 px-3 py-3 wrap>
        ...
        <v-flex wrap xs24 md24 pl-md-3 pr-md-3>
            <h1>Мои ордера</h1>
            <Myorders :orders="userOrders(pair)" :pair="pair" classtable="data theme--light table-style style-1 buy-table top-panel-blocks block-style-1"  :userId="userId" scenario="myOrders" ></Myorders><!--
            <order-list classtable="data theme&#45;&#45;light table-style style-1 buy-table top-panel-blocks block-style-1" scenario='sell' :orders="orders" ></order-list>-->
        </v-flex>
    </v-layout>
</template>
...
<script>
  ...
  export default {
    name: "index",
    components: { Acc, OrderForm, Order, Myorders, PairChecker, InfoExchange },
    ...
    computed: {
      ...
      userOrders () {
        return this.$store.getters['objects/userItems']
      },
      userId () {
